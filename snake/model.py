'''
This module defines the agent playing snake.
...
class SnakeModel(tf.keras.Model):
    '''Model class, inherit from tf.keras.Model

    Arguments:
    - num_states: int, amount of state variables
    - hidden_units: linst(int), depth of hidden layers 
    - num_action: int, amount of possible actions
...
class DQN:
    '''Class to define the deep Q network
    ...
    def __init__(self, num_states, num_actions, hidden_units, gamma,
        ...
        self.gamma = gamma
        self.model = SnakeModel(num_states, hidden_units, num_actions)
...
def make_video(env, TrainNet):
    env = wrappers.Monitor(env, os.path.join(os.getcwd(), "videos"),
                           force=True)
    rewards = 0
    steps = 0
    done = False
...
def main():
    ...
    for n in range(N):
        ...
        total_reward, losses = play_game(env, TrainNet, TargetNet, epsilon,
                                         copy_step)
        ...
        if n % 100 == 0:
            LOGGER.info(
                ('episode: {}, episode reward: {}, epsilon: {}, '+
                 'avg reward (last 100): {}, episode loss: {}')
                 .format(n, total_reward, epsilon, avg_rewards, losses))
    LOGGER.info('avg reward for last 100 episodes: {}'.format(avg_rewards))
